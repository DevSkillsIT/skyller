...
const mockAnalysis = {
  ...
  tweets: [
    {
      id: 1,
      text: "Contrato de 2 anos com renovacao automatica. Cuidado com o prazo de aviso previo de 90 dias!",
    },
    {
      id: 2,
      text: "Multa de 30% e alta para o mercado. Recomendo renegociar antes da assinatura.",
    },
              ...
              <ScrollArea className="h-full pr-4">
                <ul className="space-y-2">
                  {mockAnalysis.findings.map((finding) => (
                    ...
                    >
                      <Lightbulb
                        className={`h-4 w-4 mt-0.5 flex-shrink-0 ${typeColors[finding.type as keyof typeof typeColors].split(" ")[0]}`}
                      />
              ...
              <ScrollArea className="h-full pr-4">
                <ul className="space-y-2">
                  {mockAnalysis.redacted.map((item) => (
                    <li key={item.id} className="flex items-start gap-2 p-2 rounded-md bg-muted/50">
                      <AlertTriangle
                        className={`h-4 w-4 mt-0.5 flex-shrink-0 ${typeColors[item.type as keyof typeof typeColors].split(" ")[0]}`}
                      />
              ...
              <ScrollArea className="h-full pr-4">
                <div className="prose prose-sm dark:prose-invert max-w-none">
                  {mockAnalysis.summary.split("\n").map((line, i) => {
                    if (line.startsWith("## ")) {
                      return (
                        <h2 key={i} className="text-base font-semibold mt-0 mb-2">
                          {line.slice(3)}
                        </h2>
                      );
                    }
                    if (line.startsWith("### ")) {
                      return (
                        <h3 key={i} className="text-sm font-medium mt-4 mb-1">
                          {line.slice(4)}
                        </h3>
                      );
                    }
