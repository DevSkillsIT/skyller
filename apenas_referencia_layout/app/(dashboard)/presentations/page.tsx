    ...
    <div className="aspect-video bg-white dark:bg-slate-900 rounded-xl shadow-lg border overflow-hidden">
      ...
      {slide.type === "image" && (
        <div className="h-full flex flex-col items-center justify-center p-8">
          ...
          {slide.caption && (
            <p className="text-sm text-muted-foreground mt-4">{slide.caption}</p>
          )}
        </div>
      )}
    </div>
...
export default function PresentationsPage() {
  ...
  const addSlide = (type: keyof typeof slideTypeIcons) => {
    const newSlide = {
      ...
      type: type as typeof mockSlides[0]["type"],
      title: `Novo Slide ${slides.length + 1}`,
      bullets: type === "content" ? ["Item 1", "Item 2", "Item 3"] : undefined,
      quote: type === "quote" ? "Adicione sua citacao aqui" : undefined,
      author: type === "quote" ? "Autor" : undefined,
      data: type === "chart" ? [{ label: "A", value: 50 }, { label: "B", value: 75 }] : undefined,
    }
