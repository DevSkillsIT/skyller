...
import type { Workspace, Project, Conversation } from "@/lib/mock/data"
...
export function AppSidebar({
  ...
  const visibleRecents = showAllRecents ? recentConversations : recentConversations.slice(0, VISIBLE_RECENTS)
      ...
      <Sidebar collapsible="icon" className="border-r border-border">
        <SidebarHeader className="border-b border-border p-2">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              {open && (
                <span className="font-semibold text-lg tracking-tight">SKYLLER</span>
              )}
            </div>
            {open && (
        ...
        <SidebarContent>
          <ScrollArea className="flex-1">
                      ...
                      {workspaces.map((workspace) => (
                        <DropdownMenuItem
                          ...
                          onClick={() => {
                            ...
                            onProjectChange(null)
                            router.push(`/projects?workspace=${workspace.id}`)
                          }}
            ...
            {/* PROJETOS DO WORKSPACE - Link para página de projetos */}
                    ...
                    <SidebarMenuItem>
                      <SidebarMenuButton asChild>
                        <Link href={`/projects?workspace=${currentWorkspace.id}`}>
                          ...
                          <span>Projetos de {currentWorkspace.name}</span>
              ...
              <Collapsible open={toolsExpanded} onOpenChange={setToolsExpanded}>
                <SidebarGroup className="py-1">
                  <CollapsibleTrigger asChild>
                    <SidebarGroupLabel className="group cursor-pointer hover:bg-accent/50 rounded-md flex items-center justify-between pr-2 mb-1">
                      <div className="flex items-center gap-2">
                        <LayoutGrid className="h-4 w-4" />
                        <span>Ferramentas</span>
                      </div>
                      <div className="flex items-center gap-1 text-transparent group-hover:text-foreground/60 transition-colors">
